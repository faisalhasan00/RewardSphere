...
export class WalletService {
  ...
  /**
   * Retrieves the full balance dashboard (Confirmed + Pending).
   */
  async getBalanceDashboard(userId: string) {
    return this.walletRepository.getTieredBalances(userId);
  }
  ...
  async getHistory(userId: string, skip?: number, take?: number) {
    return this.walletRepository.getTransactionHistory(userId, skip, take);
  }
}
